<script>
    $(document).ready(function(){        
		...
		$.each($('#review-form').find('tr:has(font)').find('input'),
          ...
          $("div.Popup .CloseButton").click(hideForm);    
          $("div.button.cancel").click(hideForm);
          
          $("a.ddd").click(function(){
                $('#review-form').trigger( 'reset' );
                $("div.Popup .successHide").show();
                $("div.Popup .successShow").hide();
                $("div.DarkBg").show();
                $("div.Popup").show();
                $("div.Popup").css("top",50);
                return false;
            });
    });
</script>
...
<div class="Popup" style="width: 600px; margin-left: -300px; display: none; ">
    <div class="in">
        <div class="Bg">
            <div class="t">
                <i><b> </b></i>                        
                                ...
                                <table>
                                    <tbody>
										...
										<tr>
                                            ...
                                            <td>
                                                <div>
                                                    <input style="width:134px;" type="text" name="review[captcha]" value="" autocomplete="off" />
													<img style="margin-bottom:-15px;" src="index.php?route=product/product/captcha" id="captcha" />
                                                </div>
                                            </td>
                                        </tr>
   ...
   $(document).ready(function() { 
		...
		if (err){
			...
			return;
                ...
                if ($("#phone_number").val().length<10 || $("#phone_number").val().length>32){
                    alert("Введите номер телефона с кодом города не более 32 символов");
                    return;
                }else{
                    var phn=$("#phone_number").val()
                    phn=phn.replace("-","","g").replace(" ","","g");
                    alert(phn);
                    if (!(/^\d*$/).test(phn )){
                        alert("Номер телефона должен состоять только из чисел");
                        return;
                    }
                    }
		$('#review-form').submit();
